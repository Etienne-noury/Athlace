// Correspondance entre les codes WALDEC (nomenclature officielle du Ministère
// de l'Intérieur, préfixe 011XXX = "Sports") et les fédérations sportives
// agréées (table `federations_sportives`, colonne `sigle`).
//
// Certains codes WALDEC regroupent plusieurs fédérations sous un même code
// (ex: 011010 = Aviron + Canoë-kayak). Dans ce cas, plusieurs candidats sont
// proposés et on choisit le premier dont un des `keywords` apparaît dans le
// titre ou l'objet de l'association (texte normalisé, sans accents,
// minuscule). Si aucun mot-clé ne correspond, le premier candidat de la
// liste sert de valeur par défaut.
//
// Source de la table de correspondance : cartographie WALDEC ↔ 93
// fédérations sportives françaises (Ministère des Sports).
...
// Codes qui ne désignent pas des clubs (têtes de réseau : comités, ligues)
// et doivent être exclus de l'import de clubs.
...
/**
 * Résout un code WALDEC (011XXX) + le texte libre (titre/objet) de
 * l'association vers { sigle, discipline }, ou `null` si le code n'est pas
 * un code sport reconnu.
 */
